...
function _update(item, update) {
  console.log('更新单条记录旧数据', item, "新数据",update)
...
Collection.prototype.update = function (newVal) {
  console.log('update记录', newVal,"this.data",this.data)
  if (typeof newVal != 'object') return false
  for (var key in this.data)
 ...
 * @param {Object} newVal 更新内容
 */
Collection.prototype.updateBookSingle = function (update) {
  console.log('更新单条记录')
  for (var key in this.data){
    console.log("更新记录旧数据", this.data[key], "新数据",update)
    if (this.data[key].cl_book_id === update.cl_book_id){
      _update(this.data[key], update)
    }else{
      console.log("未匹配到记录")
    }
  }
